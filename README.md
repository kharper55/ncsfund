...
The intent was to gain familiarity with the nRF52840 SoC, learn about Zephyr RTOS, and reinstill RTOS fundamentals (previous experience with FreeRTOS using ESP-IDF). These learnings will support future custom development with nRF SoCs using the nRF Connect SDK and nRF52840 device in embedded battery-powered BLE devices. Using hardware which was not specifically intended for the tutorial further benefitted my understanding as I had to port/adapt all example code for my specific hardware. This forced me to dive deeper into devicetree structure/syntax and Kconfig. Next up is the intermediate course as well as BLE fundamentals, after which I plan to focus on software + hardware development in parallel for a specific project which employs these learnings.
